...
function App() {
  ...
  const handleCurrentProjectChange = (index) => {
    setCurrentProject(projectsArray[index])
    setPage(1)
  }
      ...
      <div className="App">
        {page === 0 && <div>
          ...
          <div id="project-carousel-container">
            <Carousel items={projectsArray} onProjectClick={handleCurrentProjectChange} title='Check Out My Projects'/>
          </div>
        ...
        {page === 1 && <ProjectDetails project={currentProject} onBackClick={handleBackToHomePage}/>}
      </div>
  );
}
